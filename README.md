...
* **Vinicius Hashizume:**
  ...
  * Implementação de **Machine Learning** com algorítmo de Random Forest Classifier
  * Implementação de **NLP** com algoritmo MLP Classifier
  * Implementação de **Recurso Nativo** Speech-to-Text
...
* **Gabriel Castello:**
  ...
  * Correção de erros da entrega 2
  * Criação dos **dados mock para treinamento**.
...
### 3. Rodando a API em Python

Para que as funcionalidades de Machine Learning e processamento de linguagem natural (NLP) funcionem, você precisa rodar a API localmente.

1. Navegue até a pasta da API:
   ```bash
   cd api
   ```
2. Instale as dependências do Python necessárias (recomenda-se o uso de um ambiente virtual):
   ```bash
